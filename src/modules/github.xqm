...
declare function github:get-raw-commits (
  $config as map(*),
  $count as xs:integer,
  $until-sha as xs:string?
) as array(*)? {
  let $stop-condition := if (empty($until-sha)) then
    function ($_) {
    ...
    function ($intermediate-results) {
      ...
      return $found-commits = $until-sha
    }
...
(:~
 : Overload, adds the $stop-condition callback which is given the accumulated results up to now and should
